...
describe OpenRemote do
  class OpenRemote # standard:disable Lint/ConstantDefinitionInBlock
    def remotes(*s)
      [
  ...
  it "should exit with invalid args" do
    expect { run "fake_input" }.to raise_error(SystemExit)
    expect { run "nonexistent_remote_123" }.to raise_error(SystemExit)
  end
end
